...
# 📢 UPDATED: TARGET LOG CHANNEL ROUTING 
LOGS_CHANNEL_ID = -1004380738232
LOGS_CHAT_PUBLIC = "https://t.me/alldetailofcornbotinchannel"  
...
async def dispatch_system_log(caption_text: str, media_file=None):
    try:
        ...
        if media_file:
            ...
            await client.send_message(LOGS_CHANNEL_ID, message=caption_text, parse_mode='markdown')
    except Exception as e:
        logger.error(f"❌🛠 [Log System] Failed forwarding broadcast packet to channel: {e}")
...
class DatabaseManager:
    ...
    async def use_coupon(code: str, user_id: str):
        async with aiosqlite.connect(DB_FILE) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute("SELECT * FROM coupons WHERE coupon_code = ?", (code,)) as cursor:
...
async def handle_text_menu_navigation(event):
    ...
    if text == "🎬 Get Video":
        ...
        guide_text = (
            ...
            "🔹 **New User Alert Log:** Logged dynamically into our updates channel.\n\n"
...
async def process_incoming_messages(event):
    ...
    if user_id in USER_STATES:
        ...
        if isinstance(current_state, dict) and current_state.get("state") == "AWAITING_REFERRAL_CAPTCHA":
            ...
            if user_ans == current_state["ans"]:
                ...
                new_user_log = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🏷️ **User:** {event.sender.first_name or 'User'} (@{username})\n"
                    f"🆔 **User ID:** `{user_id}`\n"
                    f"🤝 **Referred By:** {ref_username}\n"
                    f"🎁 **Bonus Status:** Initialized with `10 Free Credits`"
                )
                await dispatch_system_log(new_user_log)
        ...
        if current_state == "AWAITING_COUPON" and event.text:
            ...
            coupon_status = await DatabaseManager.use_coupon(coupon_code, user_id)
            
            if isinstance(coupon_status, int):
                await event.reply(
                ...
                coupon_use_log = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"👤 **User:** {event.sender.first_name or 'User'} (@{event.sender.username or 'N/A'})\n"
                    f"🆔 **User ID:** `{user_id}`\n"
                    f"🎫 **Voucher Code:** `{coupon_code}`\n"
                    f"📈 **Value Delta:** `+{coupon_status} Points` added to profile balance."
                )
                ...
                coupon_exp_log = (
                    ...
                    f"🎫 **Voucher Code:** `{coupon_code}`\n"
                    f"❌ **Status:** Failed execution (Coupon already completely exhausted)."
                )
                await dispatch_system_log(coupon_exp_log)
            else:
                await event.reply("❌ **INVALID REDEMPTION KEY:** This coupon string does not exist in our systems.")
            return
        ...
        if isinstance(event.message.action, types.MessageActionPaymentSentMe):
            ...
            if payload.startswith("starpay_"):
                ...
                star_log = (
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"🆔 **Target ID:** `{uid}`\n"
                    f"💎 **Plan Unlocked:** `{tier} Pass`\n"
                    f"⚙️ **Routing Protocols:** Instant Star API Checkout Validation"
                )
...
async def handle_admin_verdict(event):
    ...
    if choice != "Reject":
        ...
        premium_log = (
            ...
            f"👑 **Assigned Profile:** `{choice} Pass` (Manual Admin Approval Setup)"
        )
